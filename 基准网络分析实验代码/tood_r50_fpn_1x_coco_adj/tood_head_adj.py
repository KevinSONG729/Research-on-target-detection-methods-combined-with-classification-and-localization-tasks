...
class TOODHead_ADJ(ATSSHead):
    ...
    def __init__(self,
        ...
        super(TOODHead_ADJ, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.reg_decomp = TaskDecomposition(self.feat_channels,
                                            ...
                                            self.stacked_convs * 8,
                                            self.conv_cfg, self.norm_cfg)
        self.tood_cls = nn.Sequential(
            nn.Conv2d(self.feat_channels, self.num_base_priors * self.cls_out_channels, 3, padding=1), # num_base_priors 每个特征点产生锚框的数量。
            nn.Sigmoid())
        self.tood_reg = nn.Sequential(
            nn.Conv2d(self.feat_channels, self.num_base_priors * 4, 3, padding=1))
