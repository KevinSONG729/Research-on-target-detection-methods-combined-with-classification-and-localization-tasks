...
class TaskDecomposition(nn.Module):
    ...
    def forward(self, feat, avg_feat=None):
        b, c, h, w = feat.shape
        conv_weight = self.reduction_conv.conv.weight.reshape(
                1, self.feat_channels, self.stacked_convs, self.feat_channels)
        conv_weight = torch.repeat_interleave(conv_weight, b, dim=0).reshape(b, self.feat_channels,
                                          self.in_channels)
...
class TOODHead_nolayer(ATSSHead):
    ...
    def __init__(self,
        ...
        super(TOODHead_nolayer, self).__init__(num_classes, in_channels, **kwargs)
