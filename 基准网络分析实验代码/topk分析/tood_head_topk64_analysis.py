...
logging.basicConfig(level=logging.DEBUG,
                    format='%(message)s',
                    filename='/root/autodl-tmp/code/work_dirs/tood_r50_fpn_1x_coco_topk64_analysis/topk_analysis.log',
                    filemode='w')
...
class TOODHead_topk64_analysis(ATSSHead):
    ...
    def __init__(self,
        ...
        super(TOODHead_topk64_analysis, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _get_target_single(self,
        ...
        logging.debug("gt bbox is " + str(gt_bboxes.tolist()))
        if(gt_bboxes_ignore!=None):
            logging.debug("gt ignore bbox is " + str(gt_bboxes_ignore.tolist()))
        logging.debug("gt label is" + str(gt_labels.tolist()))
        ...
        # logging.debug("anchors[:,0] shape is" + str(anchors[:,0].size()))
        anchors_center = torch.cat([((anchors[:,0]+anchors[:,2])/2).unsqueeze(1), ((anchors[:,1]+anchors[:,3])/2).unsqueeze(1)], dim=1)
        ...
        if len(pos_inds) > 0:
            ...
            logging.debug("pos anchors is" + str(anchors_center[pos_inds, :].tolist()))
            logging.debug("pos anchors label is" + str(sampling_result.pos_assigned_gt_inds.tolist()))
        ...
        logging.debug("norm alignment metrics is" + str(norm_alignment_metrics[pos_inds].tolist()))
