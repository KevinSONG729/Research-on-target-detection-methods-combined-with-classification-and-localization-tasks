...
class TaskDecomposition(nn.Module):
    ...
    def forward(self, feat, avg_feat=None):
        ...
        # 将weight与卷积核相乘，使得在做1*1卷积操作的同时做了注意力的乘积，减少计算开销。
...
class TOODHead_refinev2(ATSSHead):
    ...
    def __init__(self,
                 ...
                 initial_loss_cls=dict(
                     ...
                     loss_weight=1.0),
                 **kwargs):
        assert anchor_type in ['anchor_free', 'anchor_based', 'anchor_refine']
        self.num_dcn = num_dcn
        self.anchor_type = anchor_type
        self.epoch = 0  # which would be update in SetEpochInfoHook!
        super(TOODHead_refinev2, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):
            if i < self.num_dcn:
                conv_cfg = dict(type='DCNv2', deform_groups=4) # 可变卷积,将原通道打包成几份,每份单独拥有各自的一对偏移。
            else:
                conv_cfg = self.conv_cfg
            chn = self.in_channels if i == 0 else self.feat_channels # chn(channels num)
        ...
        self.tood_cls = nn.Conv2d(
            self.feat_channels,
            self.num_base_priors * self.cls_out_channels, # num_base_priors 每个特征点产生锚框的数量。
            3,
            padding=1)
        self.tood_reg = nn.Conv2d(
            self.feat_channels, self.num_base_priors * 4, 3, padding=1)

        self.tood_reg_refine = nn.Conv2d(
    ...
    def init_weights(self):
        ...
        normal_init(self.tood_reg_refine, std=0.01)
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            cls_score = sigmoid_geometric_mean(cls_logits, cls_prob) # sqrt(P.sigmoid()*M.sigmoid())
            ...
            elif self.anchor_type == 'anchor_refine':
                reg_refine = scale(self.tood_reg_refine(reg_feat)).float()
                reg_refine = reg_refine.permute(0, 2, 3, 1).reshape(-1, 8)
                reg_dist_origin = reg_refine[:,:4]
                reg_bbox1 = self.bbox_coder.decode(
                    anchor, reg_dist_origin).reshape(b, h, w, 4).permute(0, 3, 1, 2) / stride[0]
                anchor_refine = self.bbox_coder.decode(anchor, reg_dist_origin)
                reg_dist_refine = reg_refine[:,4:]
                reg_bbox2 = self.bbox_coder.decode(anchor_refine, reg_dist_refine).reshape(
                    b, h, w, 4).permute(0, 3, 1, 2) / stride[0]
                reg_bbox = torch.cat([reg_bbox1, reg_bbox2], dim=1)

            else:
                raise NotImplementedError(
                    f'Unknown anchor type: {self.anchor_type}.'
                    f'Please use `anchor_free` or `anchor_based` or `anchor_refine`.')
            reg_offset1 = self.reg_offset_module(feat)
            reg_offset2 = self.reg_offset_module(feat)
            bbox_pred1 = self.deform_sampling(reg_bbox[:,:4,:,:].contiguous(),
                                             reg_offset1.contiguous())
            bbox_pred2 = self.deform_sampling(reg_bbox[:,4:,:,:].contiguous(),
                                             reg_offset2.contiguous())  
            bbox_pred = torch.cat([bbox_pred1, bbox_pred2], dim=1)                            
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, labels, label_weights,
        ...
        if self.epoch < self.initial_epoch:
            ...
            loss_cls = cls_loss_func(
            cls_score, targets, alignment_metrics, label_weights, avg_factor=1.0)
    ...
    def loss(self,
        ...
        Args:
            ...
            bbox_preds (list[Tensor]): Decoded box for each scale
                level with shape (N, num_anchors * 8, H, W) in
        ...
        anchor_list1, valid_flag_list1 = self.get_anchors(
            featmap_sizes, img_metas, device=device)
        anchor_list2, valid_flag_list2 = self.get_anchors(
            featmap_sizes, img_metas, device=device)
        ...
        flatten_bbox_preds1 = torch.cat([
            bbox_pred[:,:4,:,:].permute(0, 2, 3, 1).reshape(num_imgs, -1, 4) * stride[0]
            for bbox_pred, stride in zip(bbox_preds,
                                         self.prior_generator.strides)
        ], 1)
        flatten_bbox_preds2 = torch.cat([
            bbox_pred[:,4:,:,:].permute(0, 2, 3, 1).reshape(num_imgs, -1, 4) * stride[0]
            for bbox_pred, stride in zip(bbox_preds,
                                         self.prior_generator.strides)
        ], 1)

        cls_reg_targets1 = self.get_targets(
            flatten_cls_scores,
            flatten_bbox_preds1,
            anchor_list1,
            valid_flag_list1,
        ...
        (anchor_list1, labels_list1, label_weights_list1, bbox_targets_list1,
         alignment_metrics_list1) = cls_reg_targets1

        cls_reg_targets2 = self.get_targets(
            flatten_cls_scores,
            flatten_bbox_preds2,
            anchor_list2,
            valid_flag_list2,
            ...
            gt_labels_list=gt_labels,
            label_channels=label_channels)
        (anchor_list2, labels_list2, label_weights_list2, bbox_targets_list2,
         alignment_metrics_list2) = cls_reg_targets2

        bbox_preds1 = [bbox_pred[:,:4,:,:] for bbox_pred in bbox_preds]
        bbox_preds2 = [bbox_pred[:,4:,:,:] for bbox_pred in bbox_preds]

        losses_cls1, losses_bbox1,\
            cls_avg_factors1, bbox_avg_factors1 = multi_apply(
                self.loss_single,
                anchor_list1,
                cls_scores,
                bbox_preds1,
                ...
                label_weights_list1,
                bbox_targets_list1,
                alignment_metrics_list1,
                self.prior_generator.strides)

        losses_cls2, losses_bbox2,\
            cls_avg_factors2, bbox_avg_factors2 = multi_apply(
                self.loss_single,
                anchor_list2,
                cls_scores,
                bbox_preds2,
                ...
                label_weights_list2,
                bbox_targets_list2,
                alignment_metrics_list2,
                self.prior_generator.strides)

        cls_avg_factor1 = reduce_mean(sum(cls_avg_factors1)).clamp_(min=1).item()
        losses_cls1 = list(map(lambda x: x / cls_avg_factor1, losses_cls1))
        cls_avg_factor2 = reduce_mean(sum(cls_avg_factors2)).clamp_(min=1).item()
        losses_cls2 = list(map(lambda x: x / cls_avg_factor2, losses_cls2))
        losses_cls = losses_cls1 + losses_cls2

        bbox_avg_factor1 = reduce_mean(
            sum(bbox_avg_factors1)).clamp_(min=1).item()
        bbox_avg_factor2 = reduce_mean(
            sum(bbox_avg_factors2)).clamp_(min=1).item()
        losses_bbox1 = list(map(lambda x: x / bbox_avg_factor1, losses_bbox1))
        losses_bbox2 = list(map(lambda x: x / bbox_avg_factor2, losses_bbox2))
        losses_bbox = losses_bbox1 + losses_bbox2
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, priors, stride in zip(
            ...
            bbox_pred = bbox_pred[4:,:,:].permute(1, 2, 0).reshape(-1, 4) * stride[0]
    ...
    def _get_target_single(self,
        ...
        """
        logging.debug("image name is " + str(img_meta['filename']))
