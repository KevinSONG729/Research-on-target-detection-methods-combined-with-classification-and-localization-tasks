...
from mmdet.core import (anchor_inside_flags, build_assigner, distance2bbox, bbox2distance,
...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename='/root/autodl-tmp/code/test.log',
                    filemode='w')
...
class TOODHead_refine_distri(ATSSHead):
    ...
    def __init__(self,
                 ...
                 reg_max=16,
                 **kwargs):
        assert anchor_type in ['anchor_free', 'anchor_refine']
        self.num_dcn = num_dcn
        self.anchor_type = anchor_type
        self.epoch = 0  # which would be update in SetEpochInfoHook!
        self.reg_max = reg_max
        super(TOODHead_refine_distri, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.tood_reg = nn.Conv2d(
            self.feat_channels, self.num_base_priors * 4 * (self.reg_max + 1), 3, padding=1)

        self.tood_reg_refine = nn.Conv2d(
            self.feat_channels, self.num_base_priors * 4 * (self.reg_max + 1) + 4, 3, padding=1)
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            if self.anchor_type == 'anchor_free':
                reg_distribution = scale(self.tood_reg(reg_feat)).float()
                reg_distribution = reg_distribution.permute(0, 2, 3, 1).reshape(-1, 4*(self.reg_max+1))
                reg_dist = self.integral(reg_distribution)
                reg_distribution = reg_distribution.reshape(b, h, w, 4*(self.reg_max+1)).permute(0, 3, 1, 2)
                reg_bbox = distance2bbox(
                    self.anchor_center(anchor) / stride[0],
                    reg_dist).reshape(b, h, w, 4).permute(0, 3, 1,
                                                          2)  # (b, c, h, w)
            # elif self.anchor_type == 'anchor_based':  #in this way, anchor_based is not helpful
            ...
            #         b, h, w, 4).permute(0, 3, 1, 2) / stride[0]
                ...
                reg_distribution = scale(self.tood_reg_refine(reg_feat)).float()
                reg_distribution = reg_distribution.permute(0, 2, 3, 1).reshape(-1, 4*(self.reg_max+2))
                reg_refine = self.integral(reg_distribution[:,:-4])
                reg_refine = torch.cat((reg_refine, reg_distribution[:,-4:]), dim=1)
                reg_distribution = reg_distribution[:,:-4].reshape(b, h, w, 4*(self.reg_max+1)).permute(0, 3, 1, 2)
                reg_dist_origin = reg_refine[:,:4]
                anchor_refine = distance2bbox(
                    self.anchor_center(anchor), reg_dist_origin)
                reg_dist_refine = reg_refine[:,4:]
                reg_bbox = self.bbox_coder.decode(anchor_refine, reg_dist_refine).reshape(
                    b, h, w, 4).permute(0, 3, 1, 2) / stride[0]

            else:
                raise NotImplementedError(
                    f'Unknown anchor type: {self.anchor_type}.'
                    f'Please use `anchor_free` or `anchor_refine`.')
            reg_offset = self.reg_offset_module(feat)
            bbox_pred = self.deform_sampling(reg_bbox.contiguous(),
                                             reg_offset.contiguous())
            bbox_pred = torch.cat((bbox_pred, reg_distribution), dim=1)
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, reg_distribution, labels, label_weights,
        ...
        Args:
            ...
            reg_distribution(Tensor): (N, num_anchors * 4 * (reg_max + 1), H, W)
        ...
        if len(pos_inds) > 0:
            ...
            pos_reg_distribution = reg_distribution[pos_inds]

            pos_decode_bbox_pred = pos_bbox_pred
            pos_decode_bbox_targets = pos_bbox_targets / stride[0]
            pred_corners = pos_reg_distribution.reshape(-1, self.reg_max + 1)
            target_corners = bbox2distance(pos_anchor_centers, pos_bbox_targets, self.reg_max).reshape(-1)
            # regression loss
        ...
        return loss_cls, loss_bbox, loss_dfl, alignment_metrics.sum(), \
            pos_bbox_weight.sum()
    ...
    def loss(self,
        ...
        Args:
            ...
            bbox_preds (list[Tensor]): Decoded box for each scale
                level with shape (N, num_anchors * (4 + 4*(reg_max + 1)), H, W) in
        ...
        reg_distributions = [bbox_pred[:,4:,:,:] for bbox_pred in bbox_preds]
        bbox_preds = [bbox_pred[:,:4,:,:] for bbox_pred in bbox_preds]
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, priors, stride in zip(
            ...
            bbox_pred = self.integral(bbox_pred.permute(1, 2, 0)) * stride[0]
