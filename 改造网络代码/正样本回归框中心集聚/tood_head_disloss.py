...
class TOODHead_DISLOSS(ATSSHead):
    ...
    def __init__(self,
        ...
        super(TOODHead_DISLOSS, self).__init__(num_classes, in_channels, loss_dis, **kwargs)
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            logging.debug('feat is' + str(feat))
            logging.debug('cls_feat is' + str(cls_feat))
            logging.debug('cls_logits is' + str(cls_logits))
            logging.debug('cls_prob is' + str(cls_prob))
            cls_score = sigmoid_geometric_mean(cls_logits, cls_prob) # sqrt(P*M)
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, labels, label_weights,
                    bbox_targets, alignment_metrics, img_size, stride):
        ...
        if self.epoch < self.initial_epoch:
            ...
            if(torch.isnan(cls_score).any()):
                loss_cls = torch.tensor(0, dtype=torch.float, device=cls_score.device)
            else:
                loss_cls = cls_loss_func(
        ...
        logging.debug('bg_class_ind is' + str(bg_class_ind))
        logging.debug('labels shape is' + str(labels.size()))
        pos_inds = ((labels >= 0)&(labels < bg_class_ind)).nonzero().squeeze(1)
        logging.debug('pos_inds is' + str(pos_inds))
        if len(pos_inds) > 0:
            ...
            if self.epoch < self.initial_epoch:
                loss_dis = torch.tensor(0.0, dtype=torch.float)
            else:
                dis_loss_func = self.loss_dis
                loss_dis = dis_loss_func(pos_targets.contiguous(), pos_decode_bbox_pred.clone(), pos_decode_bbox_targets.contiguous(), stride[0])
        else:
            loss_bbox = bbox_pred.sum() * 0
            pos_bbox_weight = bbox_targets.new_tensor(0.)
            loss_dis = torch.tensor(0.0, dtype=torch.float)

        return loss_cls, loss_bbox, loss_dis, alignment_metrics.sum(), \
            pos_bbox_weight.sum()
    ...
    def loss(self,
        ...
        logging.debug('cls_score is' + str(cls_scores))
        ...
        # print(img_metas)
        img_size_list_tmp = [[img['batch_input_shape'][0], img['batch_input_shape'][1]] for img in img_metas]
        img_size_list_tmp = torch.tensor(img_size_list_tmp, device=torch.device('cuda:0'))
        img_size_list = [img_size_list_tmp for _ in range(0, len(anchor_list))]

        losses_cls, losses_bbox, losses_dis,\
        ...
        return dict(loss_cls=losses_cls, loss_bbox=losses_bbox, loss_dis=losses_dis)
